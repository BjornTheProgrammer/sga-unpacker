use std::{io::{BufRead, Read, Seek, SeekFrom}, path::Path, sync::{Arc, Mutex}};

use anyhow::Result;

use crate::{sga::entires::{SgaEntries, SgaFolderEntry}, utils::read_c_string};
...
impl FolderNode {
    ...
    pub fn read_files_from_folder<T: Read + BufRead + Seek>(this: Arc<Mutex<Self>>, reader: &mut T, entries: &SgaEntries) -> Result<Vec<FileNode>> {
        ...
        for i in entry.file_start_index..entry.file_end_index {
            ...
            let node = FileNode::new(file_name, entries.header.data_offset + file_entry.data_offset, file_entry.compressed_length as usize, file_entry.uncompressed_size as usize, file_entry.storage_type.clone(), this.clone());
    ...
    pub fn folder_from_entry<T: Read + BufRead + Seek>(reader: &mut T, entries: &SgaEntries, folder_entry: &SgaFolderEntry, parent: Option<Arc<Mutex<FolderNode>>>) -> Result<Self> {
        reader.seek(SeekFrom::Start(entries.header.header_blob_offset + entries.header.string_offset as u64 + folder_entry.name_offset as u64))?;
